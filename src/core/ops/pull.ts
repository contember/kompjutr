...
import { type MergeBehavior, merge } from "./merge.js";
...
function bounded(value: string, label: string, limit: number): string {
  ...
  if (value.length > limit || utf8.encode(value).length > limit) {
    throw new GitError("E2BIG", `${label} exceeds ${limit} bytes`);
  }
  return value;
}

function fullBranchRef(value: string, label: string): string {
  const boundedValue = bounded(value, label, MAX_PULL_REF_BYTES);
  const fullRef = boundedValue.startsWith("refs/") ? boundedValue : `${HEADS}${boundedValue}`;
  return requireBranchRef(bounded(fullRef, label, MAX_PULL_REF_BYTES));
}

function pullUrl(value: string): Pick<PullPlan, "url" | "displayUrl"> {
  const boundedUrl = bounded(value, "pull URL", MAX_PULL_URL_BYTES);
  let parsed: URL;
  try {
    parsed = new URL(boundedUrl);
  } catch {
  ...
  const url = normalizeRemoteUrl(boundedUrl);
  parsed.username = "";
  parsed.password = "";
  parsed.search = "";
  parsed.hash = "";
...
function requireRemoteName(value: string): string {
  const remote = bounded(value, "pull remote", MAX_PULL_REMOTE_BYTES);
  if (remote === ".") {
...
function fastForwardOptions(
  ...
  const configuredFf = configured(repo, "pull.ff", 16);
  ...
  return { fastForward: parseBooleanConfig(configuredFf, "pull.ff") };
}

function requireMergeStrategy(repo: Repository): void {
  const rebase = configured(repo, "pull.rebase", 16);
...
export function resolvePull(repo: Repository, options: PullOptions = {}): PullPlan {
  ...
  const configuredRemote =
    options.remote === undefined
      ? configured(repo, `branch.${branch}.remote`, MAX_PULL_REMOTE_BYTES)
      : undefined;
  ...
  const configuredMerge =
    options.remoteRef === undefined
      ? configured(repo, `branch.${branch}.merge`, MAX_PULL_REF_BYTES)
      : undefined;
  ...
  const configuredUrl =
    options.url === undefined
      ? configured(repo, `remote.${remote}.url`, MAX_PULL_URL_BYTES)
      : undefined;
...
export async function pull(
  ...
  const afterFetchHead = repo.head();
  ...
  return merge(
    ...
    worktree,
    {
      ...
      ...(options.message === undefined
        ? { message: defaultPullMessage(plan) }
