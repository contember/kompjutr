...
function marks(db: TestDatabase, repoId: number) {
  return db.all<{
    ...
    `SELECT oid, expanded, edge_cursor
...
describe("maintenance reachability", () => {
  ...
  it("marks a packed delta without an edge to its in-pack base", async () => {
    const { db, checkout, store } = open();
    ...
    expect(advanceMaintenanceReachability(store.shared)).toMatchObject({
      processedOid: pack.targetOid,
      discoveredObjects: 0,
    });
    expect(marks(db, checkout.repoId).map((row) => row.oid)).toEqual([pack.targetOid]);
    drain(db, store.shared);
  });
  ...
  it("accepts a repository-owned promised blob as an unmarked tree-edge terminal", () => {
    ...
    expect(marks(db, checkout.repoId)).toEqual([{ oid: tree, expanded: 1, edge_cursor: 1 }]);
    expect(
      db.scalar<number>(
  ...
  it("ignores valid-looking corrupt and oversized commit cache rows in favor of raw headers", () => {
    ...
    expect(progress).toMatchObject({ processedOid: root, discoveredObjects: 2 });
  ...
  it("uses raw loose and packed commit headers regardless of derived cache qualification", async () => {
    ...
    expect(progress).toMatchObject({ processedOid: packedCommit, discoveredObjects: 1 });
    ...
    const queuePlan = db.all<{ detail: string }>(
      `EXPLAIN QUERY PLAN
       SELECT oid FROM git_maintenance_objects
        ...
        ORDER BY oid COLLATE BINARY LIMIT 2`,
      checkout.repoId,
    );
