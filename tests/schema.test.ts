...
import { initializeFsSchema, ROOT_INODE } from "../packages/do/src/fs/schema.js";
import { SqliteGitDatabase } from "../packages/git/src/store/index.js";
import { initializeGitSchema, SCHEMA_VERSION } from "../packages/git/src/store/schema/schema.js";
...
const TABLE_OWNERSHIP = new Map<string, "global" | "shared" | "checkout">([
  ...
  ["git_integration_workspaces", "shared"],
  ["git_integration_objects", "shared"],
  ["git_integration_object_chunks", "shared"],
  ["git_integration_tree_entries", "shared"],
  ["git_integration_plans", "shared"],
  ["git_integration_plan_entries", "shared"],
  ["git_integration_reservations", "shared"],
  ["git_integration_touched", "shared"],
...
const EXPECTED_SCHEMA_OBJECTS: readonly SchemaObject[] = [
  ...
  { type: "table", name: "git_integration_object_chunks" },
  { type: "table", name: "git_integration_objects" },
  { type: "table", name: "git_integration_plan_entries" },
  { type: "table", name: "git_integration_plans" },
  { type: "table", name: "git_integration_reservations" },
  { type: "table", name: "git_integration_touched" },
  { type: "table", name: "git_integration_tree_entries" },
  { type: "table", name: "git_integration_workspaces" },
...
const EXPECTED_TABLE_COLUMNS: readonly (readonly [string, readonly string[]])[] = [
  ...
  ["git_objects", ["repo_id", "oid", "type", "size"]],
  [
  ...
  [
    ...
    ["repo_id", "workspace_id", "tree_oid", "ordinal", "name", "mode", "child_oid"],
  ],
  [
    "git_integration_plans",
    ["repo_id", "workspace_id", "plan_id", "kind", "source_rows", "entry_count"],
  ],
  ["git_integration_plan_entries", ["repo_id", "workspace_id", "plan_id", "path", "descriptor"]],
  [
    "git_integration_reservations",
    ["repo_id", "workspace_id", "plan_id", "family", "path", "descriptor"],
  ],
  [
    "git_integration_touched",
    [
      "repo_id",
      "workspace_id",
      "plan_id",
  ...
  it("enforces authenticated merge-origin operation constraints", () => {
    ...
    const insert = `INSERT INTO git_operation_state
     ...
     VALUES (?, 'merge', 'refs/heads/main', ?, 'ready', NULL, ?, ?, NULL, NULL,
             'no-commit', ?, 0, 0, 'HEAD', 'topic', '', NULL, NULL, NULL, NULL, 0, 0, 0)`;
    const oid = "1".repeat(40);

    expect(() => db.run(insert, 17, oid, oid, "2".repeat(40), null)).toThrow();
    db.run(insert, 17, oid, oid, "2".repeat(40), "merge");
