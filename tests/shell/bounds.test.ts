...
function everything(root: string): ScanEntry[] {
  ...
  for (;;) {
    const page = fs.scan(root, after === undefined ? { limit: 1000 } : { after, limit: 1000 });
    all.push(...page);
...
describe("a glob too long for SQLite to narrow with", () => {
  ...
  it("expands to the same answer the narrowed path gives", async () => {
  ...
  it("searches a deep root, where the include narrowing degrades too", async () => {
    ...
    const run = await shell.run(`grep -rl --include=*.ts NEEDLE /repo/${LONG}`);
...
describe("a tree bigger than one discovery page", () => {
  ...
  it("cp -r copies every file, not just the first page", async () => {
    expect((await shell.run("cp -r /repo/src /repo/copy")).exitCode).toBe(0);
  ...
  it("cp -r carries directories and symlinks across the boundary", async () => {
  ...
  it("cp -r never reads file bodies into the shell", async () => {
    ...
    expect((await guardedShell.run("cp -r /repo/src /repo/copy")).exitCode).toBe(0);
    expect(fs.stat("/repo/copy/f01199.ts")?.type).toBe("file");
  });
  it("rm -r removes it whatever the page size", async () => {
    expect((await shell.run("rm -r /repo/src")).exitCode).toBe(0);
    expect(fs.stat("/repo/src")).toBeNull();
    expect(everything("/repo")).toHaveLength(0);
  });
  it("a search pages through all of it", async () => {
    ...
    expect((await shell.run("grep -rl NEEDLE /repo/src")).stdout).toBe("/repo/src/zz-last.ts\n");
  });
});
describe("argument expansion bounds", () => {
  it("accepts the exact path cap and fails closed on the first path beyond it", async () => {
    fs.writeFiles(
    ...
    expect((await shell.run("true /repo/glob/*.txt")).exitCode).toBe(0);
    fs.writeFile("/repo/glob/f10000.txt", new Uint8Array(0));
    const run = await shell.run("echo /repo/glob/*.txt");
...
describe("retained-memory bounds", () => {
  ...
  it("caps stderr independently from stdout", async () => {
    ...
    const run = await bounded.run("warn");
    expect(run.stderr).toHaveLength(32);
    expect(run.stdout).toBe("");
    expect(run.truncated).toBe(true);
  });
  it("accepts stderr at its exact public limit", async () => {
    const warn: Command = (context) => {
  ...
  it("streams multi-file cat and redirects beyond the retained limit", async () => {
    fs.writeFiles([
    ...
    expect(await small.run("cat large")).toMatchObject({ stdout: "x".repeat(40), exitCode: 0 });
  });
  it("accepts xargs at the exact limit and rejects the first byte over", async () => {
    const bounded = createShell({
      fs,
      cwd: "/repo",
    ...
    expect(await bounded.run("chunks 12345678 | xargs echo")).toMatchObject({
      stdout: "12345678\n",
      exitCode: 0,
    });
    const over = await bounded.run("chunks 123456789 | xargs echo");
    expect(over.exitCode).toBe(2);
    expect(over.stderr).toContain("retained-memory limit");
  });
  it("accepts a sort buffer at its exact limit and rejects the next input byte", async () => {
    const one: Command = () =>
      result(
        (function* () {
          yield encode("a\n");
    ...
    expect(await bounded.run("one | sort")).toMatchObject({ stdout: "a\n", exitCode: 0 });
    expect((await bounded.run("two | sort")).exitCode).toBe(2);
  });
  it("releases an unstarted sort buffer before the next command reserves memory", async () => {
    ...
    expect(await bounded.run("echo 'b a' | sort | head -0; reserve")).toMatchObject({
      stdout: "",
      stderr: "",
      exitCode: 0,
  ...
  it("accepts line carry at its exact limit and rejects the next input byte", async () => {
    const eight: Command = () =>
      result(
        (function* () {
          yield encode("1234");
    ...
    expect(await bounded.run("eight | grep 1")).toMatchObject({
      stdout: "12345678\n",
      exitCode: 0,
    });
    expect((await bounded.run("nine | grep 1")).exitCode).toBe(2);
  });
  it("bounds growing head and tail probes at their exact limits", async () => {
    ...
    expect(await headBounded.run("head -1 probe")).toMatchObject({ stdout: "xxxx", exitCode: 0 });
    expect(await tailBounded.run("tail -1 probe")).toMatchObject({ stdout: "xxxx\n", exitCode: 0 });
    fs.writeFile("/repo/probe", ENCODER.encode("xxxxx"));
    expect((await headBounded.run("head -1 probe")).exitCode).toBe(2);
    expect((await tailBounded.run("tail -1 probe")).exitCode).toBe(2);
  });
  it("rolls a redirect back when an upstream retained limit fails", async () => {
    ...
    const run = await bounded.run("fail-late > target");
    expect(run.exitCode).toBe(2);
    expect(new TextDecoder().decode(fs.readFile("/repo/target"))).toBe("old");
  });
  it("does not normalize an injected coded exception during a redirect", async () => {
    fs.writeFiles([{ path: "/repo/target", bytes: ENCODER.encode("old") }]);
    ...
    await expect(bounded.run("broken > target")).rejects.toThrow("injected failure");
    expect(new TextDecoder().decode(fs.readFile("/repo/target"))).toBe("old");
  });
});
describe("git destination preflight", () => {
  ...
  it("does not charge dropped Git stderr to a zero-byte sink", async () => {
    ...
    const run = await bounded.run("git push 2>/dev/null");
