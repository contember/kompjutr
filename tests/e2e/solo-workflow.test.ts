...
describe("solo workflow", () => {
  it("grows an unborn repository into its first commit", async () => {
    ...
    // KNOWN DIVERGENCE: `init()` makes no directory (packages/git/src/ops/repository/init.ts), so
  ...
  it("keeps ignored paths untracked until add is forced", async () => {
    ...
    // kompjutr skips it and reports success (packages/git/src/ops/staging/staging.ts, above
