...
const GIT_MUTATION_BOUNDARY_INVENTORY = Object.freeze({
  Git: Object.freeze([
    "add",
    "branch",
    "branchDelete",
...
describe("public Git mutation boundary inventory", () => {
  it("freezes every supported Git-domain mutation category before wrapping", () => {
    expect(GIT_MUTATION_BOUNDARY_INVENTORY).toMatchObject({
      GitCliRunner: ["runCli"],
      free: expect.arrayContaining(["updateRef", "worktreeAdd"]),
      sqliteGitDatabase: expect.arrayContaining(["createRepository", "destroyRepository"]),
    });
    expect([
      CheckoutStore.name,
      Repository.name,
      SharedRepoStore.name,
      SqliteGitDatabase.name,
    ]).toEqual(["CheckoutStore", "Repository", "SharedRepoStore", "SqliteGitDatabase"]);
  });
});
