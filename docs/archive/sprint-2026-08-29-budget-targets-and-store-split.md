...
|---|---|---|---|
| Sprint integration | Fundamental behavior and storage-module structure change across nearly every Git path. | Hard-block review plus both parity harnesses, target benchmarks, public package smoke and the closure suite. Review blocks only a correctness, data-safety, public API, SQL/transaction/cache-ownership regression or a failing required gate; all other findings become backlog. | Any SQL/schema/public API change, new refusal, unresolved memory evidence, or benchmark target miss. |
...
| WU7–WU10 | Pure moves, but private ownership, cycles and transaction seams are fundamental. | One hard-block review per WU, focused family witnesses, public export snapshot and routine gate. Defer naming, layout, duplication, optimization and non-regression hardening findings to backlog. | SQL text/ordering, transaction scope, cache owner, public type or runtime import graph changes. |
...
- **WU6g (2026-08-30).** Commits `15cea97` and `81a990d` replaced fixed store/cache ceilings with operation-owned reservations, restored native add page durability, and added leased cgroup evidence. All root shards, 124 pack tests, 46 protocol tests, filesystem 460/460, shell 323/323, e2e 96/96, smoke 167/167, typecheck, Biome, build, and package smoke passed. Twelve calibration/capped memory scenarios passed under the 512 MiB cgroup; the highest capped reset process transient was 74,506,240 bytes. The monolithic root run hit a Vitest RPC timeout after 420/420 assertions, so closure used the same smaller shards. Extreme packed-graph scaling and internal ownership/SQL-row hardening graduated to [backlog 63](../backlog/63-bound-packed-dependency-graph-traversal.md) and [backlog 64](../backlog/64-harden-owned-reads-and-blob-id-sql-rows.md).
- **Remaining review policy (2026-08-30).** By user direction, WU7–WU10 and sprint integration use hard-block-only review. Only correctness, data safety, public API, SQL/transaction/cache ownership and required-gate failures block; every deferrable finding is recorded in backlog instead of extending the review loop.
- **WU7 (2026-08-30).** Commit `e6b9287` made `src/sqlite/store.ts` an explicit compatibility facade and mechanically moved contracts, database routing, shared-repository state, and checkout implementation under `src/sqlite/store/`. Runtime exports matched the before snapshot byte-for-byte; all 283 top-level declarations, 200 SQL literals, transaction bodies, registries, class identities, and external imports were preserved. Typecheck, build, 68 focused tests, and smoke 167/167 passed; hard-block review was clean. Grounding also confirmed that WU8–WU10 cross a substantially larger ops/journal and transaction seam than the original plan implied, so each remaining extraction must be re-grounded rather than treated as an assumed clean table-family move.
