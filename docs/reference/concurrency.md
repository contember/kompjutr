...
| Local refs, index, worktree, and journals | paused async owner × synchronous local call | Disjoint state normally `coexist`s. Ref and journal transitions publish atomically; bounded index replacement may publish several synchronous pages and its interrupted convergence is `unfenced`. An active journal rejects guarded public operations; a journal created after a network owner starts can coexist until an owner-specific recheck. |
