...
| Pull snapshot | `pull` × HEAD/upstream/journal/index/worktree mutation | Changed HEAD or upstream configuration produces `stale-reject` after fetched state is retained. An active journal at invocation produces `active-reject`; pull rechecks merge state after fetch. Overlapping staged or dirty paths reject integration while unrelated staged and dirty paths `coexist`. |
...
Clone, fetch, and push propagate one `AbortSignal` through discovery,
