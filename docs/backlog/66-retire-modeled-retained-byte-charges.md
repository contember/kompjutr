...
**Summary.** Four operations still charge a hand-computed estimate of a
...
byte budget only when it charges bytes the operation actually holds. These four
...
For push, rename detection, and selected staging, delete the byte
