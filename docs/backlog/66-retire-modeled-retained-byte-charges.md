...
|---|---|---|
| `src/git/ops/push/push-plan-types.ts` | `COMMIT_ENTRY_BYTES = 512`, `MAP_ENTRY_BYTES = 128`, `SET_ENTRY_BYTES = 96`, `pushPlanStringBytes = 48 + len*2`, plus a `PushRetainedTracker` with named parts, `transfer`, and `keepOnly` | `MAX_PUSH_COMMITS = 512`, `MAX_PUSH_OBJECTS = 100_000`, `MAX_PUSH_BRANCH_TARGETS = 1_024` |
| `src/git/ops/status/status-full.ts`, `status-clean.ts` | `SET_ENTRY_BYTES = 48` | **none** — `STATUS_RETAINED_BYTES` is the only bound on the tracked-path set |
| `src/git/ops/status/rename-detection.ts` | `RENAME_CANDIDATE_FIXED_BYTES = 256`, `RENAME_CONFIG_BYTES = 16` | `MAX_EXACT_RENAME_CANDIDATES = 10_000` |
| `src/git/ops/rebase/rebase-plan.ts` | `PENDING_REBASE_STEP_BYTES = 336` | `maxSteps`; the byte limit defaults to `Number.MAX_SAFE_INTEGER`, so it bounds nothing unless a caller sets it |
| `src/git/ops/staging/staging-selected-validation.ts` | eight `*_FIXED_BYTES` / `*_SLOT_BYTES` constants | `ADD_SELECTED_PATHS = 1_000` |
...
Acceptance: no `_BYTES` constant under `src/git/ops/` stands for a JavaScript
...
`src/git/ops/push/push-plan-*.ts`, `src/git/ops/status/status-full.ts`,
`src/git/ops/status/rename-detection.ts`, `src/git/ops/rebase/rebase-plan.ts`,
`src/git/ops/staging/staging-selected*.ts`, their tests, and
