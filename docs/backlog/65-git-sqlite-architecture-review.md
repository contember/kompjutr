...
|---|---|---|---|
| ARCH-8 | Three-way integration refuses more than 1,000 changed paths even though deleting the cap alone would leave retained plan state unbounded. | A 1,001-path integration succeeds under a real retained-state bound or a restart-safe streaming plan. | `packages/git/src/ops/integration/integration.ts`, `packages/git/src/ops/integration/integration-structure.ts` |

## Store and maintenance work

| IDs | Problem | Acceptance | Touch points |
|---|---|---|---|
| ARCH-9 | Pack ingest temporarily publishes a pending pack as complete to insert commit projections. | Ingest can publish projections for its own pending pack without exposing complete-pack visibility, including after interruption. | `packages/git/src/store/pack/pack-ingest-index.ts`, `packages/git/src/store/trees/commits.ts`, `packages/git/src/store/pack/` |
...
| ARCH-16 | Loose classification and sweep do not persist keyset cursors and repeatedly scan settled or young rows. | Both phases resume from durable OID cursors, root drift clears them, and visited rows grow linearly with candidates. | `packages/git/src/store/maintenance/sweep.ts`, `packages/git/src/store/maintenance/state.ts`, `packages/git/src/store/maintenance/roots.ts` |
...
| ARCH-22 | `git_pack_objects` duplicates every payload column from `git_pack_entries`. | One physical entry table plus a narrow canonical OID-to-location projection preserves reads, shadowing, and deletion. | `packages/git/src/store/schema/schema.ts`, `packages/git/src/store/pack/pack-ingest-index.ts`, `packages/git/src/store/pack/` |
...
| ARCH-26 | `CheckoutStore` forwards shared-repository operations already available through its guarded shared store. | Checkout exposes checkout-owned behavior only; shared calls preserve active-store guards through `store.shared`. | `packages/git/src/store/checkout/checkout.ts`, `packages/git/src/store/repository/shared.ts` |
...
| ARCH-37 | The documented maintenance terminal-result contract disagrees with immediate rollover when `nextEligibleAt` is null. | Code, tests, and reference docs define one behavior for null and non-null future boundaries. | `packages/git/src/ops/repository/maintenance.ts`, `docs/reference/git-support.md` |
| ARCH-38 | `maintenance()` discards repack advancer status and can report discarded work as progress after root drift. | The detecting call exposes or performs the restart and does not report discarded work as forward progress. | `packages/git/src/ops/repository/maintenance.ts`, `packages/git/src/store/maintenance/repack.ts` |
