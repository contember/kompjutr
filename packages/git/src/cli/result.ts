...
const USAGE = {
  ...
  pull: "usage: git pull [--rebase|--ff|--no-ff|--ff-only] [<remote> [<branch>]]\n",
  ...
  "rev-parse": "usage: git rev-parse [<options>] <args>...\n",
  branch:
  ...
  reset:
    "usage: git reset [--mixed | --hard] [<commit>]\n   or: git reset [<commit>] -- <pathspec>...\n",
  checkout:
    "usage: git checkout [-f] <branch>\n   or: git checkout -b <new-branch> [<start-point>]\n   or: git checkout <tree-ish> -- <pathspec>...\n",
  switch: "usage: git switch <branch>\n   or: git switch -c <new-branch>\n",
  restore: "usage: git restore [--source=<tree>] <pathspec>...\n",
  merge: "usage: git merge --continue | --abort\n",
};
...
export function gitCliRevisionRequired(context?: GitCliOutputContext): GitCliResult {
  return gitCliDiagnosticResult("fatal: Needed a single revision\n", "", "", 128, context);
}

export function gitCliStdoutPartsResult(
  ...
  for (const part of parts) {
    const partBytes = gitCliUtf8ByteLength(part, "git CLI stdout", false);
    if (partBytes > maximum - bytes) {
      throw new GitError("E2BIG", `git CLI stdout exceeds ${maximum} bytes`);
    }
    bytes += partBytes;
    chunks.push(part);
  }
  return gitCliResult(chunks.join(""), stderr, exitCode);
}
